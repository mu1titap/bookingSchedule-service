...
//
    @Operation(summary = "세션 참가 등록" , description = "- 멘토링 세션 참가 요청 <br>" +
            ...
            " 참고 : 인자 중에서 mentoringName 는 직접적으로 멘토링 세션 참가 등록 기능에 필요한 데이터는 아니지만 스케줄 read data 업데이트에 필요함<br>f"
            ,tags = {"멘토링 세션 참가"})
    @PostMapping("")
    ...
    @Operation(summary = "멘토링 서비스에게로 세션 정보 가져오는 feign client 테스트 Api" ,description = "입력 인자 : uuid(세션 uuid) "
            ,tags = {"feignClient"})
    @GetMapping("/feign-client-test")
    public BaseResponse<SessionResponseOutDto> SessionResponseOutDto(@RequestParam("uuid") String uuid) {
        return new BaseResponse<>( mentoringServiceCallUseCase.getSessionOutDtoByUuid(uuid));
    }
